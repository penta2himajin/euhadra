...
L1 で es を評価する際の ASR は **NVIDIA Canary-180M-Flash via [`istupakov/canary-180m-flash-onnx`](https://huggingface.co/istupakov/canary-180m-flash-onnx)** (CC-BY 4.0、INT8 ~213 MB)。`scripts/setup_canary_es.sh` でダウンロード、`eval_l1_smoke --canary-es-dir <path> --langs es` で起動。アーキテクチャ詳細・代替の Parakeet-TDT-0.6B-v3-multi へのフォールバック計画は `docs/canary-integration.md`。

初回実測 (FLEURS-es 10 utt、INT8): CER 9.75%、RTF 0.091。8/10 utt は ≤7% CER、外れ値 2 件 (チャンク欠落) 除外で 8-utt 平均 1.7% CER (≈ 2.5% WER)。Canary-180M-Flash の公式 model card は **MLS Spanish WER 3.17 %** / **MCV-16.1 ES WER 4.90 %** で、サンプル拡張 + WER 化により直接比較する予定 (`docs/canary-integration.md` の「End-to-end validation」節)。
