...
**上限を測る指標である点は変わらない。** 合成無音は実録音の環境音とスペクトルが違い、この条件は energy VAD に有利に働く。出荷 5 言語（en / ja / es / zh / ko）は測定・CI ゲート済み（#137 / #150 / #156 / #157）。
...
- [x] ΔWER 測定の CI 配線 — `evaluate (VAD ΔWER)` ジョブ。既定構成のみを Δ 固定閾値でゲート（#137）。出荷 5 言語は #150 / #156 / #157
