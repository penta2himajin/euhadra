...
ただし AED 特有の残課題がある。`src/canary/decoder.rs` の `min_token_to_frame_ratio` は出力長が `0.2 × T_speech` に達するまで EOS を `-inf` にする。`T_speech` はアダプタ側のエネルギー推定（または呼び出し側が渡す `speech_encoder_frames`）で、録音全体の `T_sub` ではない（#136）。対称の上限 `max_token_to_frame_ratio`（既定 0.8）で無音時の反復ループも切る。`SpeechOnly` は無音をエンコーダに渡さないため実害をさらに小さくする。
