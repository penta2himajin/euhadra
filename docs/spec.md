...
| L3 | フィラー除去の直接 F1 | en / ja / zh / ko（**es は gold 未整備**） | なし |
...
#### 帰結 1: 実装済みと測定済みは一致しない

`SpanishFillerFilter` は実装されているが es のフィラー gold は存在しない。実際この穴のせいで、**フィラーの直後にカンマがあると何も除去しない**という欠陥が長期間検出されなかった（句読点なしの CIEMPIESS 書き起こしでのみ検証していたため）。発覚したのは 5 言語を手で流したときである。

新しい言語のフィルタや処理を追加する際は、gold と baseline を同時に用意するか、**測定されていないことを明示する**。

#### 帰結 2: モデルの対応言語数は選定基準として弱い
