...
- `TermDictionary` — ユーザー登録の用語置換（エイリアス列挙、依存ゼロ）。**誤認識の訂正ではなく用語の置換**である点が `PhonemeCorrector` と異なる。ASR が「タイプライター」と正しく転写しても、話者が `typwrtr` と書きたい場合に効く。§3.6.1 参照
...
#### 3.6.1 用語置換辞書（`TermDictionary`）

**訂正ではなく置換。** ASR が「タイプライター」を出したとき、それは誤認識ではなく正しい転写である。話者が `typwrtr` と書きたいだけであり、音響モデルをどれだけ改善しても解決しない。決められるのは話者本人だけである。

したがって `PhonemeCorrector`（音素距離による誤認識訂正）とは別の機構であり、要求される言語資源も違う。`PhonemeCorrector` は CMUdict と `<en_us>` 固定の G2P に依存するため実質 en 専用だが、`TermDictionary` はエイリアスを列挙する方式なので**全言語で同じように動く**。
...
**euhadra は辞書を同梱しない。** 同梱した時点で「話者が何を意図したか」についての editorial な立場を取ることになる（#122 で引いた線）。`load(path)` も提供しない——`TermEntry` が `Deserialize` を導出するだけで、形式はアプリが決める。ファイル形式を euhadra が決めるのはアプリの設定設計への越権である。

**それでもパイプライン内に置く理由は順序である。** 置換自体は数十行でアプリ側にも書けるが、`BasicPunctuationRestorer` の後では文頭大文字化済みのテキストに当たり、`InverseTextNormalizer` の後では書き換え済みの数詞に当たる。**位置を選べるのはパイプラインの中にいるものだけ**で、これが唯一の設計上の必然性である。

##### 照合と畳み込み

`MatchPolicy::for_language` が言語から選ぶ。`FillerFilter::for_language`（§3.5）と同じ理由——組み合わせを手で行うと事故が静かに起きる。
...
| ja | 部分文字列 | ✓ | ✓ | ✓ |
| zh | 部分文字列 | ✓ | ✓ | — |
| `MatchPolicy::none()` | 部分文字列 | — | — | — |

**規則: 情報を失う畳み込みは入れない。** ひらがなとカタカナは同じ語だが、「タイプライタ」と「タイプライター」は必ずしも同じ語ではない（製品名と一般語が長音符だけで違いうる）ので長音符は畳まない。es のアクセント除去は `año`/`ano` を同一視するため入れない。zh の簡繁変換は Unicode 操作ではなく OpenCC 規模の写像表を要するため入れない。**入れなかった畳み込みはエイリアスを 1 行足せば済む**——正規化は正しさの機構ではなく入力量の最適化にすぎない。

`Language` に無い言語は `MatchPolicy::none()` を**明示的に**使う。`for_language` がサイレントにフォールバックしない理由は `Language::from_bcp47` が未対応言語に `None` を返すのと同じである。

##### 動作の保証

- **1 パス、左から最長一致。置換済み領域は再走査しない。** 再走査すると、term が他エントリの alias でもある場合に連鎖し、結果がエントリ順に依存する
- **一致したら必ず置換する。** 信頼度スコアも文脈判定も持たない。「タイプライター」を本当にその意味で言った日も置換されるが、これは意図的な取引である——報告された `Correction` は取り消せるが、静かに起きなかった置換は復元できない
- **構築時に弾く**: 空 term / 空 alias / term と同一の alias / 畳み込み後 2 文字未満 / 別 term との alias 衝突。全件を集約して返す（アプリの登録 UI が一覧表示できるように）
- **`Correction.span` はコードポイント単位**（`Span` の他の用途と同じ）。UTF-16 やバイトオフセットは消費者が変換する

##### 測定

**§11.4 の既存3分類に入らない第4のカテゴリ。** gold にあたるものが「ネイティブ話者の判断」ではなく「ユーザーが登録した内容」なので、言語知識を必要としない。したがって測るのは品質ではなく**仕様適合**であり、決定的なユニットテストで足りる。

これは他の Tier 2 処理と対照的である。句読点復元や自己訂正検出は「何が正しいか」に言語判断が要り、だから gold と F1 が要る。用語置換に必要なのは「登録した通りに置換したか」だけで、それは検査可能である。

##### 既知の限界

2 文字の alias は通る。`IT` を登録した英語話者は全ての `it` が書き換わる。**長さは代理指標であり、本当の危険は頻度である**——`Qz` は 2 文字でも無害で、`IT` が危険なのは `it` が高頻度だからにすぎない。頻度で警告するには言語ごとの語頻度リストが要り、§11.4 でいう「中央で広げられない」per-language データに落ちるため実装していない。
...
| `speech_pad` | 200 ms | 境界の前後に残す音声。語頭・語尾の欠けは無音より高くつく。左端は `preroll` と `max` を取り、直前セグメント終端でクランプする |
| `preroll` | **400 ms** | 検出 onset より前の履歴からの巻き戻し。`speech_pad` より広く取り、境界ガードで前発話への食い込みを防ぐ（#144） |
| `max_speech` | 30 s | 一切休まない話者への安全弁。唯一意図的に過分割する設定 |

左端の実効余白は `max(speech_pad, preroll)` であり、両者を加算しない。開始位置は `max(onset − 余白, 直前セグメント終端, 0)`。履歴バッファはセッションが保持するサンプル列そのもので、`Segmenter` はサンプルインデックスと境界ガードだけを持つ（ファイル経路・マイク経路で同一規則）。
...
#### 「refine」が指しうる三つの概念（#146）

文書・議論で混同しやすいので、用語を固定する。

| 概念 | 何をするか | 現状 |
|---|---|---|
| **`FinalPass`** | セッション終了時の final の**入力ソース**を選ぶ（上記） | 出荷済み |
| **後追い再デコード**（仮称 `SecondPassDecode`） | 区間確定後も、無音ギャップ等を条件にまとめた音声を**非同期でもう一度 ASR**し、精度の高い清書で置き換える。LLM ではない | **未実装・見送り**（API スケッチと測定が先。#146） |
| **`LlmRefiner`（Tier 3）** | 最終テキストの言い回し・文脈補正。**ASR を再実行しない** | トレイトのみ（#122 保留） |

後追い再デコードを入れる場合の要件（実装時）:

- ホットパス（次の発話の確定）を塞がない
- 再デコード結果が短い・空など明らかに劣る場合は、先の確定を捨てない
- グループ内で言語が混在するときに単一言語での一括再デコードを強いない
- `FinalPass` / `Session::partials` / Tier 3 との関係を本節の表に追随して更新する

`FinalPass::SpeechOnly` の「もう一度 ASR」はセッション終了時のソース選択であり、上記の後追い再デコードではない。
...
**上限を測る指標である点は変わらない。** 合成無音は実録音の環境音とスペクトルが違い、この条件は energy VAD に有利に働く。`es` / `zh` は測定・CI ゲート済み（#150 / #156）。`ko` は未測定（#157）。

#### 判断ゲート: テキスト側の除去は不要

#133 の分岐は「VAD 導入後も幻覚が残るか」だった。**残らない。** よって言語別ブラックリストには踏み込まない——§11.4 帰結 3 でいう「中央で広げられない」領域に落ちる対処を避けられた。

ただし AED 特有の残課題がある。`src/canary/decoder.rs` の `min_token_to_frame_ratio` は出力長が `0.2 × T_speech` に達するまで EOS を `-inf` にする。`T_speech` はアダプタ側のエネルギー推定（または呼び出し側が渡す `speech_encoder_frames`）で、録音全体の `T_sub` ではない（#136）。対称の上限 `max_token_to_frame_ratio`（既定 1.5）で無音時の反復ループも切る。発話が大半のクリップでは推定を捨ててフル長を使い、clean 経路の truncation を避ける。`SpeechOnly` は無音をエンコーダに渡さないため実害をさらに小さくする。
...
[Tier 2: TextProcessor]  ← LLM 不要、数十ミリ秒、5〜250MB ONNX
    ...
    │  句読点挿入（ルールベース）                      ✅ 実装済み
    │  用語置換（ユーザー辞書、エイリアス列挙）        ✅ 実装済み
    │  - TermDictionary: 訂正ではなく置換。全言語で同じ挙動
...
- [x] TermDictionary（ユーザー登録の用語置換、エイリアス列挙、依存ゼロ、全言語）
...
- [x] `FinalPass` / 後追い再デコード / `LlmRefiner` の用語分離（#146）。後追い再デコード本体は未実装
...
- [x] ΔWER 測定の CI 配線 — `evaluate (VAD ΔWER)` ジョブ。既定構成のみを Δ 固定閾値でゲート（#137）。`es` / `zh` は #150 / #156
- [ ] ko の ΔWER — Dolphin（#157）
