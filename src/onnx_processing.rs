...
fn embed_one(
    session: &mut Session, tokenizer: &Tokenizer, text: &str,
) -> Result<Array1<f32>, String> {
    let enc = tokenizer.encode(text, true).map_err(|e| format!("tokenize: {e}"))?;
    let len = enc.get_ids().len();

    let ids = Array2::from_shape_vec((1, len), enc.get_ids().iter().map(|&x| x as i64).collect()).unwrap();
    let mask = Array2::from_shape_vec((1, len), enc.get_attention_mask().iter().map(|&x| x as i64).collect()).unwrap();
    let tids = Array2::from_shape_vec((1, len), enc.get_type_ids().iter().map(|&x| x as i64).collect()).unwrap();
    ...
    let arr = outputs[0].try_extract_array::<f32>().map_err(|e| format!("extract: {e}"))?;
    let view = arr.view();
...
impl OnnxEmbeddingFilter {
    pub fn load(model_dir: impl AsRef<Path>) -> Result<Self, FilterError> {
        ...
        let mut session = Session::builder()
            ...
            .map_err(|e| FilterError { message: format!("load model: {e}") })?;

        let tokenizer = Tokenizer::from_file(dir.join("tokenizer.json"))
            .map_err(|e| FilterError { message: format!("load tokenizer: {e}") })?;

        let pure_fillers: Vec<String> =
            ["um","uh","uhm","umm","hmm","er","ah","eh"].iter().map(|s| s.to_string()).collect();
        let contextual_fillers: Vec<String> =
            ["so","well","basically","actually","literally","right"].iter().map(|s| s.to_string()).collect();

        let mut filler_embeddings = Vec::new();
        for f in &pure_fillers {
            filler_embeddings.push(
                embed_one(&mut session, &tokenizer, f)
        ...
        Ok(Self {
            ...
            filler_embeddings, pure_fillers, contextual_fillers,
            pure_threshold: 0.82,
        })
    }

    fn max_filler_sim(&self, emb: &Array1<f32>) -> f32 {
        self.filler_embeddings.iter().map(|f| emb.dot(f)).fold(f32::NEG_INFINITY, f32::max)
    }
    ...
    fn is_sentence_initial(words: &[&str], idx: usize, removed: &[bool]) -> bool {
        ...
        for j in (0..idx).rev() {
            ...
            return words[j].ends_with('.') || words[j].ends_with('!') || words[j].ends_with('?') || words[j].ends_with(',');
        }
        true
    }
}
...
impl TextFilter for OnnxEmbeddingFilter {
    async fn filter(&self, text: &str) -> Result<FilterResult, FilterError> {
        ...
        if words.is_empty() {
            return Ok(FilterResult { text: String::new(), removed: vec![] });
        }

        let mut session = self.session.lock().await;
        let mut embeddings = Vec::with_capacity(words.len());
        for w in &words {
            embeddings.push(embed_one(&mut session, &self.tokenizer, w)
                .map_err(|e| FilterError { message: e })?);
        }
        drop(session);
        ...
        for i in 0..n {
            let clean: String = words[i].to_lowercase().chars().take_while(|c| c.is_alphanumeric()).collect();
            if self.max_filler_sim(&embeddings[i]) >= self.pure_threshold && self.pure_fillers.contains(&clean) {
        ...
        for i in 0..n {
            ...
            let clean: String = words[i].to_lowercase().chars().take_while(|c| c.is_alphanumeric()).collect();
            if self.contextual_fillers.contains(&clean) && Self::is_sentence_initial(&words, i, &removed_flags) {
                removed_flags[i] = true;
                labels.push(words[i].to_string());
            }
        }

        let kept: Vec<&str> = words.iter().enumerate().filter(|(i,_)| !removed_flags[*i]).map(|(_,w)| *w).collect();
        Ok(FilterResult { text: kept.join(" "), removed: labels })
    }
}
...
impl OnnxPunctuationRestorer {
    pub fn load(
        model_path: impl AsRef<Path>, tokenizer_path: impl AsRef<Path>, labels: Vec<String>,
    ) -> Result<Self, ProcessError> {
        let session = Session::builder()
            .and_then(|mut b| b.commit_from_file(model_path.as_ref()))
            .map_err(|e| ProcessError { message: format!("load model: {e}") })?;
        let tokenizer = Tokenizer::from_file(tokenizer_path.as_ref())
            .map_err(|e| ProcessError { message: format!("load tokenizer: {e}") })?;
        Ok(Self { session: Arc::new(Mutex::new(session)), tokenizer: Arc::new(tokenizer), labels })
    }
    ...
    pub fn default_labels() -> Vec<String> {
        ["OU","OO",".O","!O",",O",".U","!U",",U",":O",":U",";O",";U","'O","'U","-O"]
    ...
    fn parse_label(label: &str) -> (Option<char>, bool) {
        ...
        if chars.len() < 2 { return (None, false); }
        let punct = if chars[0] == 'O' { None } else { Some(chars[0]) };
...
impl TextProcessor for OnnxPunctuationRestorer {
    async fn process(&self, text: &str, _ctx: &ContextSnapshot) -> Result<ProcessResult, ProcessError> {
        let words: Vec<&str> = text.split_whitespace().collect();
        if words.is_empty() {
            return Ok(ProcessResult { text: String::new(), corrections: vec![] });
        }
        ...
        let ids_val = Value::from_array(ids).map_err(|e| ProcessError { message: format!("{e}") })?;
        let mask_val = Value::from_array(mask).map_err(|e| ProcessError { message: format!("{e}") })?;

        let outputs = session.run(vec![
            ("input_ids", ids_val.into_dyn()),
            ("attention_mask", mask_val.into_dyn()),
        ]).map_err(|e| ProcessError { message: format!("inference: {e}") })?;
            ...
            if let Some(wid) = wid_opt {
                ...
                if w < words.len() && !seen[w] && ti < seq_len {
                    ...
                    let best = (0..num_labels)
                        .max_by(|&a, &b| logits_owned[offset + a].partial_cmp(&logits_owned[offset + b]).unwrap())
                        .unwrap();
        ...
        Ok(ProcessResult { text: result, corrections })
    }
}
